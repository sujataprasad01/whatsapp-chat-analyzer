...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        most_used_emoji=helper.most_used_emoji(selected_user, df)

        col1, col2=st.columns(2)

        with col1:
            st.dataframe(most_used_emoji)

        with col2:
            fig, ax=plt.subplots()
            ax.pie( most_used_emoji[1].head(), labels=most_used_emoji[0].head(), autopct="%0.2f")
            st.pyplot(fig)
